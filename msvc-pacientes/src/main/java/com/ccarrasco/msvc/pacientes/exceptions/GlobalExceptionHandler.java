package com.ccarrasco.msvc.pacientes.exceptions;


import com.ccarrasco.msvc.pacientes.dtos.ErrorDTO;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDTO> handlePacienteException(PacienteException exception){
        ...
        if(exception.getMessage().contains("no se encuentra en la base de datos")) {
            ...
            Map<String, String> errorMap = Collections.singletonMap("Paciente no encontrado", exception.getMessage());
            ...
            Map<String, String> errorMap = Collections.singletonMap("Paciente existente", exception.getMessage());
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(this.createErrorDTO(HttpStatus.CONFLICT.value(), new Date(), errorMap));
        }
    }

    @ExceptionHandler(FichaPacienteException.class)
    public ResponseEntity<ErrorDTO> handleFichaPacienteException(FichaPacienteException exception){

        if(exception.getMessage().contains("no se encuentra en la base de datos")) {
            // Esto nos sirve para cuando no existe en la base de datos la ficha paciente
            Map<String, String> errorMap = Collections.singletonMap("Ficha Paciente no encontrada", exception.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(this.createErrorDTO(HttpStatus.NOT_FOUND.value(), new Date(), errorMap));


        }else{
            // Esto nos sirve para cuando la ficha paciente ya existe en nuestra base de datos
            Map<String, String> errorMap = Collections.singletonMap("Ficha Paciente existente", exception.getMessage());
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(this.createErrorDTO(HttpStatus.CONFLICT.value(), new Date(), errorMap));
        }
    }
